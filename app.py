...
#       "user_id": data.get("user_id"),
        "user_id": data.get("client_id"),       # pull from the form’s client_id field
...
def try_reconnect():
    ...
    # Determine which parameters to use (saved vs. fallback)
    if last_connection_params and all(last_connection_params.values()):
        params = last_connection_params
    else:
        app.logger.warning("No saved params for auto-reconnect; falling back to CONFIG")
        params = {
            "ip":           CONFIG["IBKR_IP"],
            "user_id":      CONFIG["IBKR_USER_ID"],
            "account_type": CONFIG["IBKR_ACCOUNT_TYPE"]
        }

    app.logger.info(f"Auto-reconnect using params: {json.dumps(params)}")
    result, error = send_backend_request("connect", json_data=params)
